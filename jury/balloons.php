...
foreach ($cdatas as $cdata) {
    if (isset($cdata['freezetime']) &&
        ...
        echo "<div class=\"alert alert-info\"><i class=\"fas fa-snowflake\"></i> Scoreboard of c${cdata['cid']} (${cdata['shortname']}) is now frozen.</div>\n\n";
    }
}
...
if (!empty($BALLOONS)) {
    ...
    foreach ($BALLOONS as $row) {
        ...
        echo '<td class="probid">' .
            '<i class="fas fa-golf-ball" style="color: ' .
            specialchars($probs_data[$row['probid']]['color']) .
            ';"></i> ' . specialchars($row['probshortname']) . '</td>';
        ...
        foreach ($TOTAL_BALLOONS[$row['teamid']] as $prob_solved) {
            echo '<div title="' . specialchars($prob_solved) .
                '" class="fas fa-golf-ball" style="color: ' .
