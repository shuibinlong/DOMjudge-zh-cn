...
<div class="container">
<?php if (! $row['valid']): ?>
<div class="alert alert-warning">本次提交被裁判忽略，因此无法确定您的结果。</div>
<?php endif; ?>

<div class="d-flex flex-row">
<div class="p-2">题目ID：<b><span class="probid"><?=specialchars($row['shortname'])?></span> -
    <?=specialchars($row['probname'])?></b></div>
<div class="p-2">提交时间：<b><?=printtime($row['submittime'], null, $row['cid'])?></b></div>
<div class="p-2">语言：<b><?=specialchars($row['langname'])?></b></div>
<div class="p-2">编译结果：
<?php if ($row['result'] == 'compiler-error'): ?>
<span class="badge badge-danger">编译失败</span></div>
</div>
<?php else: ?>
<span class="badge badge-success">编译成功</span></div>
</div>
<div class="d-flex flex-row">
<div class="p-2">评测结果：<?php echo printresult($row['result'], true)?></div>
</div>
<?php endif; ?>
