...
function putScoreBoard($cdata, $myteamid = null, $static = false, $filter = false, $sdata = null)
{
    ...
    if ($fdata['showfinal']) {
        if (empty($cdata['finalizetime'])) {
            $moreinfo = "暂时的排名情况，并不是最终结果";
        } else {
            $moreinfo = "最终的排名情况";
        }
    } elseif ($fdata['stopped']) {
        $moreinfo = "比赛结束，请耐心等待最终的排名情况";
    ...
    if ($fdata['showfrozen']) {
        ...
        if (IS_JURY) {
            ...
            $warning = "<strong>排行榜冻结提示：</strong>比赛已经进入了最后 $timerem 分钟的封榜阶段 —— 在比赛最后 $timerem 分钟内的所有提交，将只会在排行榜中显示为 pending 状态。";
        }
